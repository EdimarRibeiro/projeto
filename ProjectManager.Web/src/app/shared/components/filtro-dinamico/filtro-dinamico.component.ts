...
export class FiltroDinamicoComponent {
  ...
  public tabelaLista = [
    { table: "Projeto", id: "id" },
    { table: "ProjetoResponsavel", id: "id" },
    { table: "Usuario", id: "id" },
  ];

  public usuarioId = 1;
  public usuarioTemConfiguracao = false;
  ...
  getConfiguracaoUsuario() {
    ...
    this.db.getByKey(this.configuracao?.tabela, this.usuarioId).then((filtroUsuario) => {
      if (filtroUsuario) {

        filtroUsuario.dataSourceLista ? this.loadListaPesquisa(filtroUsuario.dataSourceLista) : this.loadFiltroUsuario(filtroUsuario.formulario);
      }
    }), error => {
      this.configuracaoPadrao(this.configuracao);
    };
  ...
  loadFiltroUsuario(filtroUsuario) {
    this.usuarioTemConfiguracao = true;
    this.configurarOperacaoAtivas(filtroUsuario.tipo);
    this.definirFiltroUsuario(filtroUsuario);
  }
  ...
  adicionarFiltroBanco(filtro, tipoFiltro) {
    let salvarFiltro: any = { id: this.usuarioId };
  ...
  definirFiltroUsuario(filtroSalvo) {
  ...
  excluirFiltroUsuario() {
    this.db.delete(this.configuracao?.tabela, this.usuarioId);
  }
}
