...
namespace ProjectManager.Domain.Utils.Expressions
{
    public static class IQueryableExtensions
    {
        ...
        public static PagedResult<TResult> TakePage<T, TResult>(this IQueryable<T> source, Condition<T> condition, Pagination pagination, Expression<Func<T, TResult>> select)
        {
            ...
            result.TotalPages = result.TotalRecords / pagination.Limit;
            result.Records = source.Skip(pagination.Offset).Take(pagination.Limit).Select(@select).ToArray();
        ...
        public static PagedResult<T> TakePage<T>(this IQueryable<T> source, Condition<T> condition, Pagination pagination)
        {
            ...
            result.TotalPages = result.TotalRecords / pagination.Limit;
            result.Records = source.Skip(pagination.Offset).Take(pagination.Limit).ToArray();
        ...
        public static PagedResult<T> ToPagedResult<T>(this IQueryable<T> source, Pagination pagination)
        {
            if (pagination == null) throw new ArgumentNullException(nameof(pagination));

            var totalRecords = source.Count();
            var totalPages = totalRecords / (double)pagination.Limit;
            var result = new PagedResult<T>
            {
